trigger HandleOrderUpdate on mghw__Invoice_Statement__c (after update) {

    // Create a map of IDs to all of the *old* versions of sObjects
    // updated by the call that fires the trigger.
    Map<ID, mghw__Invoice_Statement__c> oldMap = new Map<ID, mghw__Invoice_Statement__c>(Trigger.old);
    ...
    for (mghw__Invoice_Statement__c invoice: Trigger.new) {
    ...
    if (invoiceIds.size() > 0) {
        OrderFullfillment.postOrder(invoiceIds);
    }
 }